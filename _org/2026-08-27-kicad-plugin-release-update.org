...
11. [[https://paul356.github.io/2026/08/07/kicad-plugin-vision-pdf.html][基于KiCad的大语言模型插件(Agent) — 支持 Vision 模型与 PDF 文本提取]]
...
- 板边安装焊盘：连接器槽位的 ~Edge.Cuts~ 画在封装内部（~fp_line~ / ~fp_rect~），旧代码只收集顶层 ~gr_*~ 外框，把连接器位置误判为板外，直接拒绝布线
...
[[/images/kicad-plugin-wexin-group-chat5.png]]
