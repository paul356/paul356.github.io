...
[[https://github.com/freerouting/freerouting][freerouting]] 是一个成熟的开源自动布线工具，使用 Specctra DSN/SES 格式与 KiCad 交换数据。KiCad 可以将 PCB 导出为 DSN 文件，freerouting 读取 DSN 后完成布线并输出 SES 文件，KiCad 再将 SES 导入回 PCB。
...
1. 确保 PCB 编辑器中已打开需要布线的 .kicad_pcb 文件，且封装已摆放到位。
   - [[/images/kicad-plugin-autoroute-before-route.png]]
2. 在 KiCad AI Assistant 插件窗口的菜单栏中选择 =Tools → Auto Route…=。
   - [[/images/kicad-plugin-autoroute-menu.png]]
3. 插件会弹出网络选择对话框，列出板上所有网络。可以勾选不希望 freerouting 自动布线的网络（例如 GND、电源网络等，通常这些网络需要手动铺铜处理）。如果不选择任何网络，则对全部网络布线。
   - [[/images/kicad-plugin-autoroute-skip-nets.png]]
...
6. 布线完成后，插件自动将结果导入 PCB，保存文件并刷新编辑器视图，状态栏变为"✅ Backend ready"。
   - [[/images/kicad-plugin-autoroute-result.png]]
