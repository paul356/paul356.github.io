...
[[/images/kicad-plugin-group-pcb-items.png]]

然后让大语言模型使用 =place_component_group= 分别优化各元器件组的组内布局。

[[/images/kicad-plugin-optimize-item-groups.png]]

等各元器件组内布局优化完成后，将各元器件组移动到PCB上的合适位置。

[[/images/kicad-plugin-group-optimize-result.png]]

最后使用自动布线（Auto Route）完成布线。

[[/images/kicad-plugin-group-optimize-route.png]]

*** *PCB Zone 管理工具*

|  # | 工具名                   | 作用                                      |
|----+--------------------------+-------------------------------------------|
|  1 | =list_zones=               | 列出 PCB 上所有 zones（copper_pour / keepout，即填充区/禁区） |
...
[[/images/kicad-plugin-fill-zone-request.png]]

[[/images/kicad-plugin-fill-zone-result.png]]
