...
def upload_image_from_path(image_path, image_digest):
  print("uploading image {}".format(image_path))
  #return image_path, [0, "http://{}".format(image_path)]
  client, _ = Client()
  try:
...
def get_images_from_markdown(content):
    ...
    for line in lines:
        ...
        while match:
            images.append(match.group(1)) # add matched image link
...
def upload_images_and_update(content):
    images = get_images_from_markdown(content)
    image_urls = {}
    ...
    for img in images:
        file_path = "." + img
        digest = file_digest(file_path)
        if cache_get(digest) != None:
            image_urls[img] = cache_get(digest)[1]
        else:
            (_, image_url) = upload_image_from_path(file_path, digest)
...
def process_posts(post_folder):
    ...
    for path in pathlist:
        ...
        sync_wexin = fetch_attr(content, 'sync_wexin').strip()
        if sync_wexin != "":
            print(path_str)
